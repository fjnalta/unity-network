...
public class net_PlayerSyncRotation : NetworkBehaviour {
	...
	void TransmitRotations()
	{
		if(isLocalPlayer)
		{
			if(CheckIfBeyondThreshold(playerTransform.localEulerAngles.y, lastPlayerRot) || CheckIfBeyondThreshold(camTransform.localEulerAngles.x, lastCamRot) || CheckIfBeyondThreshold(bodyTransform.localEulerAngles.y-90, lastPlayerRot))
			{
				if(!Input.GetMouseButton(1)){
					lastPlayerRot = bodyTransform.eulerAngles.y-90;
				} else {
					lastPlayerRot = playerTransform.eulerAngles.y;
				}
